...
void saveFrame(void)
{
	...
	KeyFrame[FrameIndex].rotHeda = rotHeda;
	KeyFrame[FrameIndex].rotRArm = rotRArm;
	KeyFrame[FrameIndex].rotIArm = rotIArm;
...
void resetElements(void)
{
	...
	rotHeda = KeyFrame[0].rotHeda;
	rotRArm = KeyFrame[0].rotRArm;
	rotIArm = KeyFrame[0].rotIArm;
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].rotHedaInc = (KeyFrame[playIndex + 1].rotHeda - KeyFrame[playIndex].rotHeda) / i_max_steps;
	KeyFrame[playIndex].rotRArmInc = (KeyFrame[playIndex + 1].rotRArm - KeyFrame[playIndex].rotRArm) / i_max_steps;
	KeyFrame[playIndex].rotIArmInc = (KeyFrame[playIndex + 1].rotIArm - KeyFrame[playIndex].rotIArm) / i_max_steps;
...
void animate(void)
{
	if (play)
	{
		...
		else
		{
			...
			rotHeda += KeyFrame[playIndex].rotHedaInc;
			rotRArm += KeyFrame[playIndex].rotRArmInc;
			rotIArm += KeyFrame[playIndex].rotIArmInc;
	...
	Model brazoDer, Model brazoIzq, Model cabeza, Model piso)
{
	...
	model = glm::rotate(model, glm::radians(-rotRArm), glm::vec3(1.0f, 0.0f, 0.0f));
	shader.setMat4("model", model);
	...
	model = glm::rotate(model, glm::radians(0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	model = glm::translate(model, glm::vec3(0.75f, 2.5f, 0));
	model = glm::rotate(model, glm::radians(-rotIArm), glm::vec3(1.0f, 0.0f, 0.0f));
	shader.setMat4("model", model);
	...
	model = glm::rotate(model, glm::radians(-rotHeda), glm::vec3(0.0f, 1.0f, 0.0f));
	shader.setMat4("model", model);
	cabeza.Draw(shader);
}
...
int main()
{
	...
	for (int i = 0; i < MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].rotHeda = 0;
		KeyFrame[i].rotRArm = 0;
		KeyFrame[i].rotIArm = 0;
		KeyFrame[i].rotRodDerInc = 0;
		KeyFrame[i].rotHedaInc = 0;
		KeyFrame[i].rotRArmInc = 0;
		KeyFrame[i].rotIArmInc = 0;
		KeyFrame[i].rotInc = 0;
	}
